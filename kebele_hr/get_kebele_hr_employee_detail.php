...
try {
    ...
    if (is_numeric($employee_id)) {
        $sql = "SELECT * FROM employees WHERE id = ? AND working_kebele = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("is", $employee_id, $user_kebele);
    } else {
        $sql = "SELECT * FROM employees WHERE employee_id = ? AND working_kebele = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $employee_id, $user_kebele);
    }
