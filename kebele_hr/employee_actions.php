...
switch ($action) {
    ...
    case 'delete':
        ...
        $user_kebele = $_SESSION['kebele'] ?? ''; // Assuming user's kebele is in session
        ...
        if (!$user_kebele) {
            $response['message'] = 'User kebele not set in session';
            break;
        }

        // Verify employee belongs to user's kebele
        $verify_stmt = $conn->prepare("SELECT employee_id FROM employees WHERE employee_id = ? AND kebele = ?");
        $verify_stmt->bind_param("ss", $employee_id, $user_kebele);
        $verify_stmt->execute();
        if ($verify_stmt->get_result()->num_rows === 0) {
            $response['success'] = false;
            $response['message'] = "Employee not found or unauthorized access.";
            $verify_stmt->close();
            break;
        }
        $verify_stmt->close();
