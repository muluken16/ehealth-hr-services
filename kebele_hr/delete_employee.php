...
try {
    ...
    if (is_numeric($employee_id)) {
        $check_sql = "SELECT id, employee_id, first_name, last_name, documents, scan_file, criminal_file, fin_scan, loan_file, leave_document FROM employees WHERE id = ? AND created_by = ?";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("is", $employee_id, $current_user);
    } else {
        $check_sql = "SELECT id, employee_id, first_name, last_name, documents, scan_file, criminal_file, fin_scan, loan_file, leave_document FROM employees WHERE employee_id = ? AND created_by = ?";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("ss", $employee_id, $current_user);
    }
    ...
    if (is_numeric($employee_id)) {
        $delete_sql = "DELETE FROM employees WHERE id = ? AND created_by = ?";
        $delete_stmt = $conn->prepare($delete_sql);
        $delete_stmt->bind_param("is", $employee_id, $current_user);
    } else {
        $delete_sql = "DELETE FROM employees WHERE employee_id = ? AND created_by = ?";
        $delete_stmt = $conn->prepare($delete_sql);
        $delete_stmt->bind_param("ss", $employee_id, $current_user);
    }
