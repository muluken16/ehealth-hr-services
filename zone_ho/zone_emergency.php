...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add') {
            ...
            $stmt->bind_param("sssssisss", $_POST['incident_type'], $_POST['description'], $_POST['location'], $_POST['severity'], $_SESSION['user_id'], $_POST['assigned_to'], $user_zone, $_POST['woreda'], $_POST['kebele']);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'update') {
            $stmt = $conn->prepare("UPDATE emergency_responses SET incident_type=?, description=?, location=?, severity=?, status=?, assigned_to=?, woreda=?, kebele=? WHERE id=? AND zone=?");
            $stmt->bind_param("sssssissss", $_POST['incident_type'], $_POST['description'], $_POST['location'], $_POST['severity'], $_POST['status'], $_POST['assigned_to'], $_POST['woreda'], $_POST['kebele'], $_POST['id'], $user_zone);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'delete') {
            $stmt = $conn->prepare("DELETE FROM emergency_responses WHERE id=? AND zone=?");
            $stmt->bind_param("is", $_POST['id'], $user_zone);
            $stmt->execute();
            $stmt->close();
        }
    }
    header('Location: zone_emergency.php');
    exit();
}
...
$query = "SELECT e.*, u1.name as reported_by_name, u2.name as assigned_to_name FROM emergency_responses e LEFT JOIN users u1 ON e.reported_by = u1.id LEFT JOIN users u2 ON e.assigned_to = u2.id WHERE e.zone = ?";
$params = [$user_zone];
$types = "s";
...
$staff = $conn->query("SELECT id, name FROM users WHERE (role = 'wereda_health_officer' OR role = 'kebele_health_officer') AND zone = '$user_zone' ORDER BY name");
...
<head>
    ...
    <title>HealthFirst | Zone Emergency Response</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <h1 class="page-title">Emergency Response - <?php echo htmlspecialchars($user_zone); ?></h1>
                </div>
                        ...
                        <form method="GET" class="filters-form">
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a href="zone_emergency.php" class="btn-secondary">Clear</a>
                                </div>
                            </div>
                        </form>
                    </div>
