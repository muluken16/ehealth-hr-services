...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['generate_report'])) {
    ...
    switch ($report_type) {
        case 'patients':
            ...
            $query = "SELECT COUNT(*) as total_patients,
                      ...
                      FROM patients
                      WHERE zone = ? AND created_at BETWEEN ? AND ?";
            $params = [$user_zone, $start_date . ' 00:00:00', $end_date . ' 23:59:59'];
    ...
    $stmt = $conn->prepare("INSERT INTO reports (type, title, content, generated_by, zone, woreda, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $content = json_encode($report_data);
    $stmt->bind_param("sssissss", $report_type, $report_title, $content, $_SESSION['user_id'], $user_zone, $woreda_filter, $start_date, $end_date);
    $stmt->execute();
    $report_id = $conn->insert_id;
    $stmt->close();

    // Redirect to view the report
    header('Location: zone_reports.php?view=' . $report_id);
    exit();
}

// Get existing reports
$reports = $conn->query("SELECT r.*, u.name as generated_by_name FROM reports r JOIN users u ON r.generated_by = u.id WHERE r.zone = '$user_zone' ORDER BY r.created_at DESC LIMIT 20");

// Get weredas for filter
$weredas = $conn->query("SELECT DISTINCT woreda FROM patients WHERE zone = '$user_zone' UNION SELECT DISTINCT woreda FROM appointments WHERE zone = '$user_zone' UNION SELECT DISTINCT woreda FROM inventory WHERE zone = '$user_zone' ORDER BY woreda");
...
<head>
    ...
    <title>HealthFirst | Zone Reports</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <h1 class="page-title">Reports - <?php echo htmlspecialchars($user_zone); ?></h1>
                </div>
            ...
            <div class="content">
                ...
                <?php
                $conn = getDBConnection();
                $stmt = $conn->prepare("SELECT * FROM reports WHERE id = ? AND zone = ?");
                $stmt->bind_param("is", $_GET['view'], $user_zone);
                $stmt->execute();
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="report-meta">
                            ...
                            <?php if (!empty($report['woreda'])): ?>
                            <p><strong>Wereda:</strong> <?php echo htmlspecialchars($report['woreda']); ?></p>
                            <?php endif; ?>
                        </div>
                ...
                <div class="form-actions">
                    <a href="zone_reports.php" class="btn-secondary">Back to Reports</a>
                </div>
                <?php else: ?>
