...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add') {
            ...
            $stmt->bind_param("sddssisssss", $_POST['facility_name'], $_POST['assessment_type'], $_POST['score'], $_POST['findings'], $_POST['recommendations'], $_SESSION['user_id'], $user_zone, $_POST['woreda'], $_POST['kebele'], $_POST['assessment_date'], $_POST['next_assessment_date']);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'update') {
            $stmt = $conn->prepare("UPDATE quality_assurance SET facility_name=?, assessment_type=?, score=?, findings=?, recommendations=?, woreda=?, kebele=?, assessment_date=?, next_assessment_date=? WHERE id=? AND zone=?");
            $stmt->bind_param("ssdsssssss", $_POST['facility_name'], $_POST['assessment_type'], $_POST['score'], $_POST['findings'], $_POST['recommendations'], $_POST['woreda'], $_POST['kebele'], $_POST['assessment_date'], $_POST['next_assessment_date'], $_POST['id'], $user_zone);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'delete') {
            $stmt = $conn->prepare("DELETE FROM quality_assurance WHERE id=? AND zone=?");
            $stmt->bind_param("is", $_POST['id'], $user_zone);
            $stmt->execute();
            $stmt->close();
        }
    }
    header('Location: zone_qa.php');
    exit();
}
...
$query = "SELECT q.*, u.name as assessed_by_name FROM quality_assurance q JOIN users u ON q.assessed_by = u.id WHERE q.zone = ?";
$params = [$user_zone];
$types = "s";
...
// Get assessment types and weredas for filters
$assessment_types = $conn->query("SELECT DISTINCT assessment_type FROM quality_assurance WHERE zone = '$user_zone' ORDER BY assessment_type");
$weredas = $conn->query("SELECT DISTINCT woreda FROM quality_assurance WHERE zone = '$user_zone' ORDER BY woreda");

// Get upcoming assessments
$upcoming = $conn->query("SELECT * FROM quality_assurance WHERE zone = '$user_zone' AND next_assessment_date >= CURDATE() AND next_assessment_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) ORDER BY next_assessment_date LIMIT 5");
...
<head>
    ...
    <title>HealthFirst | Zone Quality Assurance</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <h1 class="page-title">Quality Assurance - <?php echo htmlspecialchars($user_zone); ?></h1>
                </div>
                        ...
                        <form method="GET" class="filters-form">
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a href="zone_qa.php" class="btn-secondary">Clear</a>
                                </div>
                            </div>
                        </form>
                    </div>
