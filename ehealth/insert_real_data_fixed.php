...
foreach ($medicines as $medicine) {
    ...
    $stmt->bind_param("sississs", $medicine, $quantity, $expiry, $supplier, $min_stock, $zone, $woreda, $kebele);
    $stmt->execute();
    $stmt->close();
}
...
foreach ($supplies as $supply) {
    ...
    $stmt->bind_param("sisssss", $supply, $quantity, $supplier, $min_stock, $zone, $woreda, $kebele);
    $stmt->execute();
    $stmt->close();
}
...
echo "Inserted 10 emergency responses.\n";
...
echo "Inserted 5 quality assurance records.\n";
...
foreach (array_slice($employees_data, 0, 20) as $employee) { // For 20 employees
...
echo "Inserted 20 leave requests.\n";
...
echo "Inserted 8 training sessions.\n";
