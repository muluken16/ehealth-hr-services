...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add') {
            ...
            $stmt->bind_param("issssssssi", $_POST['patient_id'], $_POST['doctor_name'], $_POST['appointment_date'], $_POST['appointment_time'], $_POST['department'], $_POST['notes'], $_POST['zone'], $user_woreda, $_POST['kebele'], $_SESSION['user_id']);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'update') {
            $stmt = $conn->prepare("UPDATE appointments SET patient_id=?, doctor_name=?, appointment_date=?, appointment_time=?, department=?, status=?, notes=?, kebele=? WHERE id=? AND woreda=?");
            $stmt->bind_param("isssssssis", $_POST['patient_id'], $_POST['doctor_name'], $_POST['appointment_date'], $_POST['appointment_time'], $_POST['department'], $_POST['status'], $_POST['notes'], $_POST['kebele'], $_POST['id'], $user_woreda);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'delete') {
            $stmt = $conn->prepare("DELETE FROM appointments WHERE id=? AND woreda=?");
            $stmt->bind_param("is", $_POST['id'], $user_woreda);
            $stmt->execute();
            $stmt->close();
        }
    }
    header('Location: wereda_appointments.php');
    exit();
}
...
$query = "SELECT a.*, p.first_name, p.last_name FROM appointments a JOIN patients p ON a.patient_id = p.id WHERE a.woreda = ?";
$params = [$user_woreda];
$types = "s";
...
$patients = $conn->query("SELECT id, first_name, last_name FROM patients WHERE woreda = '$user_woreda' AND status = 'active' ORDER BY first_name");

// Get kebeles for filter
$kebeles = $conn->query("SELECT DISTINCT kebele FROM appointments WHERE woreda = '$user_woreda' ORDER BY kebele");
...
$stats = $conn->query("
    ...
    FROM appointments
    WHERE woreda = '$user_woreda' AND appointment_date >= CURDATE()
")->fetch_assoc();
...
<head>
    ...
    <title>HealthFirst | Wereda Appointments Management</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <h1 class="page-title">Appointments Management - <?php echo htmlspecialchars($user_woreda); ?></h1>
                </div>
                        ...
                        <form method="GET" class="filters-form">
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a href="wereda_appointments.php" class="btn-secondary">Clear</a>
                                </div>
                            </div>
                        </form>
                    </div>
