...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add') {
            ...
            $stmt->bind_param("ssisssisss", $_POST['item_name'], $_POST['category'], $_POST['quantity'], $_POST['unit'], $_POST['expiry_date'], $_POST['supplier'], $_POST['min_stock_level'], $_POST['zone'], $user_woreda, $_POST['kebele']);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'update') {
            $stmt = $conn->prepare("UPDATE inventory SET item_name=?, category=?, quantity=?, unit=?, expiry_date=?, supplier=?, min_stock_level=?, kebele=? WHERE id=? AND woreda=?");
            $stmt->bind_param("ssisssisss", $_POST['item_name'], $_POST['category'], $_POST['quantity'], $_POST['unit'], $_POST['expiry_date'], $_POST['supplier'], $_POST['min_stock_level'], $_POST['kebele'], $_POST['id'], $user_woreda);
            $stmt->execute();
            $stmt->close();
        } elseif ($action == 'delete') {
            $stmt = $conn->prepare("DELETE FROM inventory WHERE id=? AND woreda=?");
            $stmt->bind_param("is", $_POST['id'], $user_woreda);
            $stmt->execute();
            $stmt->close();
        }
    }
    header('Location: wereda_inventory.php');
    exit();
}
...
$query = "SELECT * FROM inventory WHERE woreda = ?";
$params = [$user_woreda];
$types = "s";
...
// Get categories and kebeles for filters
$categories = $conn->query("SELECT DISTINCT category FROM inventory WHERE woreda = '$user_woreda' ORDER BY category");
$kebeles = $conn->query("SELECT DISTINCT kebele FROM inventory WHERE woreda = '$user_woreda' ORDER BY kebele");
...
<head>
    ...
    <title>HealthFirst | Wereda Inventory Management</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <h1 class="page-title">Inventory Management - <?php echo htmlspecialchars($user_woreda); ?></h1>
                </div>
                        ...
                        <form method="GET" class="filters-form">
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <a href="wereda_inventory.php" class="btn-secondary">Clear</a>
                                </div>
                            </div>
                        </form>
                    </div>
                                ...
                                <tbody>
                                    <?php while ($item = $inventory->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td><?php echo $item['expiry_date'] ? date('M j, Y', strtotime($item['expiry_date'])) : 'N/A'; ?>
                                            </td>
                                            <td><?php echo htmlspecialchars($item['kebele']); ?></td>
                                            <td>
