...
try {
    ...
    $stmt = $conn->prepare("
        SELECT DISTINCT working_kebele as k FROM employees WHERE woreda LIKE ? AND working_kebele != ''
        UNION
        SELECT DISTINCT kebele as k FROM employees WHERE woreda LIKE ? AND kebele != ''
        ORDER BY k
    ");
    $stmt->bind_param("ss", $woreda_wildcard, $woreda_wildcard);
    $stmt->execute();
    $res = $stmt->get_result();
...
<head>
    ...
    <style>
        ...
        .add-btn:hover {
            ...
            box-shadow: 0 6px 15px rgba(26, 74, 95, 0.3);
        }
...
<body>
        ...
        <div class="hr-dashboard">
            ...
            <div class="stats-grid">
                <div class="stat-card-premium">
                    <div class="stat-icon" style="background: #eff6ff; color: #2563eb;"><i class="fas fa-users"></i>
                    </div>
                    <div class="stat-info">
            ...
            <div class="filters-section">
                ...
                <select class="filter-select" id="kebeleFilter">
                    ...
                    <?php foreach ($kebeles as $k):
                        // Remove "Kebele" prefix if it exists to avoid double label
                        $label = preg_replace('/^Kebele\s+/i', '', $k);
                        ?>
                        <option value="<?php echo htmlspecialchars($k); ?>">Kebele <?php echo htmlspecialchars($label); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <button class="print-btn" onclick="clearFilters()" title="Clear All Filters"
    ...
    <!-- Add Employee Modal REMOVED - using add_employee.php -->
    ...
    <script>
        ...
        function loadEmployees(page = 1) {
            ...
            tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;">Loading employees...</td></tr>';
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:red;">Error: ${data.message || 'Failed to load data'}</td></tr>`;
                    }
                })
                .catch(err => {
                    console.error('Fetch error:', err);
                    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:red;">Network Error: ${err.message}<br>Check browser console (F12) for details</td></tr>`;
                });
        }
                ...
                <tr class="clickable-row" onclick="viewEmployeeDetails('${emp.employee_id}')">
                    ...
                    <td data-label="Actions">
                        <div class="action-buttons">
                            ...
                            <button class="action-btn edit" onclick="event.stopPropagation(); window.location.href='edit_employee.php?id=${emp.employee_id}'" title="Edit Profile"><i class="fas fa-edit"></i></button>
            ...
            content.innerHTML = `
                ...
                <div style="padding: 10px 0 40px; display: flex; flex-direction: column; gap: 12px;">
                    <div style="display: flex; gap: 10px;">
                        <button class="submit-btn" style="flex: 2; height: 50px; font-size: 1rem; cursor: pointer;" onclick="window.location.href='edit_employee.php?id=${emp.employee_id}'"><i class="fas fa-user-edit"></i> Edit Full Profile</button>
